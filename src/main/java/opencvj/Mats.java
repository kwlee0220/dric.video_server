...
//	}
	...
	public static Mat decodeImage(byte[] image) {
		MatOfByte mob = new MatOfByte(image);
		try {
			return Imgcodecs.imdecode(mob, Imgcodecs.IMREAD_ANYCOLOR);
		}
		finally {
			mob.release();
		}
...
//	}
	...
	public static BufferedImage toBufferedImage(Mat mat) {
		...
		try {
			...
			Size sz = mat.size();
			Size2i size = new Size2i((int)sz.width, (int)sz.height);
			...
			BufferedImage bi = new BufferedImage(size.getWidth(), size.getHeight(), type);
...
//	}
	...
	public static void calcRangeMask(Mat mat, Range<Integer> range, Mat rangeMask) {
		createIfNotValid(rangeMask, mat.size(), CvType.CV_8UC1);
		Core.inRange(mat, new Scalar(range.lowerEndpoint()), new Scalar(range.upperEndpoint()), rangeMask);
	}
...
//	}
	...
	public static void scaleDepthToColoredGray(Mat mat, Scalar unknownColor, Mat colored) {
		Mat mask = new Mat();
		try {
			calcRangeMask(mat, Range.closed(1,(int)Short.MAX_VALUE), mask);
